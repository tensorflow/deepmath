...
class Model(model.Model):
  ...
  def make_embedding(self, x):
    with self.graph.as_default():
      x = keras.layers.Convolution1D(256, 1,
                                     border_mode='valid',
                                     subsample_length=1,
                                     activation='relu')(x)
      x = keras.layers.Convolution1D(512, 5,
                                     border_mode='valid',
                                     subsample_length=2,
                                     activation='relu')(x)
      x = keras.layers.Convolution1D(1024, 5,
                                     border_mode='valid',
                                     subsample_length=2,
                                     activation='relu')(x)
      x = keras.layers.Convolution1D(self.embedding_size, 5,
                                     border_mode='valid',
                                     activation='relu')(x)
